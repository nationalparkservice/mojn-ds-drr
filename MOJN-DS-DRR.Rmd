...
``` {r visitdate.timeline.1, warning = FALSE, out.width = "100%", fig.cap = capFig("Timeline of annual and 3-yr spring visits in DEVA."), fig.alt = ""}
...
This bar plot shows counts of the number of different lifeforms that were observed at springs for each park and 3-year cycle during the reporting period. Both annual and 3-year springs are included in the 3-year cycles. The black vertical line indicates the median number of different lifeforms at springs in each park, while the red dashed line indicates the mean number of different lifeforms at springs in each park.

``` {r lifeformsperspringplot, fig.height = 8, warning = FALSE, fig.cap = capFig("Bar plots showing count of number of different vegetation lifeforms observed for each park and 3-year cycle.")}
lifeformsperspringplot <- LifeformsPerSpringPlot()

if (params$isAccessible == "yes") {
  lifeformsperspringplot
} else {
  p <- ggplotly(lifeformsperspringplot, tooltip = c("text")) %>%
    layout_ggplotly(x = 0, y = 0)
...
``` {r lifeformsobservationsplot, fig.height = 10, fig.cap = capFig("Box plots showing count of vegetation lifeform categories observed for each park and 3-year cycle.")}
lifeformsobservationsplot <- MostCommonLifeformsPlot()

if (params$isAccessible == "yes") {
  lifeformsobservationsplot
} else {
  ggplotly(lifeformsobservationsplot, tooltip = "text") %>%
    layout_ggplotly(x = 0, y = -0.04)
}
```
